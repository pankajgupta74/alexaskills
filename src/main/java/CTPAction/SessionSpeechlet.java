...
public class SessionSpeechlet implements Speechlet {
    ...
    private static final String POLICY_KEY = "012345678";
    private static final String POLICY_SLOT = "policynum";
    ...
    private SpeechletResponse getWelcomeResponse() {
        // Create the welcome message.
    	  String speechText =
                  "Welcome to the Alexa Skills Kit sample. Please tell me your policy number by "
                          + "saying, my policy number is 123456789";
          String repromptText =
                  null;
        ...
        String premium = ctpServiceAction.processRequest("100001890");
        
        repromptText= "Your premium is : "+premium;
    ...
    private SpeechletResponse setColorInSession(final Intent intent, final Session session)  {
        ...
        Slot favoriteColorSlot = slots.get(POLICY_SLOT);
        String speechText, repromptText;
try{
        // Check for favorite color and create output to user.
        if (favoriteColorSlot != null) {
            // Store the user's favorite color in the Session and create response.
            String policyNo = favoriteColorSlot.getValue();
            
            CTPServiceAction ctpServiceAction = new CTPServiceAction();
            String premium = ctpServiceAction.processRequest(policyNo);
            
            session.setAttribute(POLICY_KEY, premium);
            speechText =
                    String.format("I now know that your premium is %s. You can ask me your "
                            + "premium by saying, what's my premium is?", premium);
            repromptText =
                    "You can ask me your premium due date by saying, what's my premium due date is ?";

        } else {
            // Render an error since we don't know what the users favorite color is.
            speechText = "I'm not sure what your policy number is, please try again";
            repromptText =
                    "I'm not sure what your policy number is. You can tell me your policy number "
                            + "by saying, my policy number is 123456789";
        }
        return getSpeechletResponse(speechText, repromptText, true);
}
catch(Exception e)
{
    ...
    private SpeechletResponse getColorFromSession(final Intent intent, final Session session) {
        ...
        String favoriteColor = (String) session.getAttribute(POLICY_KEY);

        // Check to make sure user's favorite color is set in the session.
        if (StringUtils.isNotEmpty(favoriteColor)) {
            speechText = String.format("Your premium is %s. Goodbye.", favoriteColor);
        } else {
            // Since the user's favorite color is not set render an error message.
            speechText =
                    "I'm not sure what your premium is. You can say, my policy number is "
                            + "123456789";
            isAskResponse = true;
        }
